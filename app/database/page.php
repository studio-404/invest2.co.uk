...
class page
{
	...
	private function selectByIdColumn($args)
	{
		$fetch = "[]";
		$idx = $args['idx']; 
		$lang = $args['lang'];
		...
		$json = Config::CACHE."pages_byidcols".str_replace(array("-"," "), "", implode("_",$_SESSION['URL'])).$args['idx'].$args['lang'].$column.".json";
		...
		if(file_exists($json)){
			...
			if($column=="photo")
			{
				...
				$select = "SELECT `".$column."` FROM `navigation` WHERE `idx`=:idx AND `lang`=:lang AND `status`!=:one";
			}
